...
func init() {
	...
	backupCmd.Flags().StringVarP(&awsProfile, "aws-profile", "a", "default", "Name of the AWS profile as provided in the credentials file. Environment variable: AWS_PROFILE")
	...
	backupCmd.Flags().StringVarP(&dynamoTableAccountID, "dynamo-table-account-id", "did", "", "AccountID that will be used to access the dynamoDB")
	...
	backupCmd.Flags().StringVarP(&dynamoTableRole, "dynamo-table-role", "dr", "OrganizationAccountAccessRole", "Role that will be used to access the dynamoDB")
	...
	backupCmd.Flags().StringVarP(&s3BucketAccountID, "s3-bucket-account-id", "sid", "", "AccountID that will be used to access the s3 Bucket")
	...
	backupCmd.Flags().StringVarP(&s3BucketRole, "s3-bucket-role", "sr", "OrganizationAccountAccessRole", "Role that will be used to access the s3 Bucket")
...
var backupCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		actions.TableBackup(dynamoTableName, dynamoBatchSize, time.Duration(waitTime)*time.Millisecond, s3BucketName, s3BucketFolderName, s3DateSuffix, awsProfile, dynamoTableAccountID, dynamoTableRole, dynamoTableRegion, s3BucketAccountID, s3BucketRole, s3BucketRegion)
	},
}
