...
func TableBackup(tableName string, batchSize int64, waitPeriod time.Duration, bucket, prefix string, addDate bool, profile, dynamoAccountID, dynamoRole, dynamoRegion, s3AccountID, s3Role, s3Region string) {
	if addDate {
		t := time.Now().UTC()
	...
	proc := core.NewAwsHelper(profile, dynamoRegion, dynamoAccountID, dynamoRole)
	dest := core.NewAwsHelper(profile, s3Region, s3AccountID, s3Role)
